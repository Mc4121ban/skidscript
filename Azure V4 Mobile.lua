...
local Window = OrionLib:MakeWindow({Name = "Azure V4", HidePremium = false, SaveConfig = true, ConfigFolder = "OrionTest"})
...
Settings.Combat.Enabled = true
Settings.Combat.Silent = true
local cac = Instance.new("ScreenGui");
		local dbrr = Instance.new("TextButton");
		local UICorner = Instance.new("UICorner");
		local UIStroke = Instance.new("UIStroke");
		cac.Name = "cac";
		cac.Parent = gethui() or game:GetService("CoreGui");
		cac.ZIndexBehavior = Enum.ZIndexBehavior.Sibling;
		cac.ResetOnSpawn = false;
		dbrr.Name = "dbrr";
		dbrr.Parent = cac;
		dbrr.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
		dbrr.BorderColor3 = Color3.fromRGB(0, 0, 0);
		dbrr.BorderSizePixel = 0;
		dbrr.Position = UDim2.new(0, 10, 0, 10);
		dbrr.Size = UDim2.new(0, 120, 0, 55);
		dbrr.Font = Enum.Font.SourceSans;
		dbrr.Text = "Aimlock";
		dbrr.TextColor3 = Color3.fromRGB(0, 0, 0);
		dbrr.TextSize = 25;
		dbrr.TextWrapped = true;
		UICorner.Parent = dbrr;
		UIStroke.Thickness = 0.2;
		UIStroke.Parent = dbrr;
		local function MGCKA_fake_script()
			local script = Instance.new("LocalScript", dbrr);
			local TweenService = game:GetService("TweenService");
			local UserInputService = game:GetService("UserInputService");
...
local function onButtonClicked()
    if Settings.Combat.Enabled then
        ...
        local temp_target = Script.Functions.GetClosestPlayer()
        
        local NewTarget = temp_target
        
        if Script.Locals.IsTargetting and NewTarget and NewTarget.Character then
        textButton.Text = "On"
                    if Settings.Combat.Alerts then
                OrionLib:MakeNotification({
                    Name = "Locked",
                    Content = "Target: " .. (NewTarget.Character.Humanoid.DisplayName or "Unknown"),
                    Image = "rbxassetid://4483345998",
                    Time = 1
})
            end
            Script.Locals.Target = NewTarget
        else
            -- Send a notification that no target was found if alerts are enabled
            if Settings.Combat.Alerts then
                OrionLib:MakeNotification({
                    Name = "No Target",
                    Content = "No valid target found or targeting disabled.",
                    Image = "rbxassetid://4483345998",
                    Time = 1
                })
                textButton.Text = "Off"
			...
			local function update(input)
				...
				local goalPos = UDim2.new(0, framePos.X.Offset + delta.X, 0, framePos.Y.Offset + delta.Y);
				local tweenInfo = TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out);
				local tween = TweenService:Create(textButton, tweenInfo, {Position=goalPos});
				tween:Play();
			end
		...
		coroutine.wrap(MGCKA_fake_script)()
